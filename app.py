...
def load_spectral_data():
    current_dir = os.path.dirname(os.path.abspath(__file__))
    # Assuming the CSV is in the same folder as app.py or inside the data folder
    csv_path = os.path.join(current_dir, "master_hyperspectral_signatures_averaged.csv")
    if not os.path.exists(csv_path):
        csv_path = os.path.join(current_dir, "data", "master_hyperspectral_signatures_averaged.csv")
...
# --- 3. MULTISPECTRAL PLOTTING ENGINE (For Map View) ---
...
# --- 4. APP NAVIGATION & SIDEBAR ---
st.sidebar.title("Navigation")
app_mode = st.sidebar.radio("Select View Module:", ["🗺️ Spatial Diagnostics Map", "📈 Hyperspectral Reflectance"])
st.sidebar.markdown("---")

if app_mode == "🗺️ Spatial Diagnostics Map":
...
# --- 6. HYPERSPECTRAL VIEWER MODULE ---
elif app_mode == "📈 Hyperspectral Reflectance":
    
    if spectral_df.empty:
        st.error("Hyperspectral CSV not found. Please ensure 'master_hyperspectral_signatures_averaged.csv' is in the root directory or data folder.")
    else:
        # Extract band column names (e.g., 'Band_1', 'Band_2')
        id_col = "generated_id"
        band_cols = [col for col in spectral_df.columns if col.startswith("Band_")]
        x_axis_labels = [int(col.split('_')[1]) for col in band_cols]
        available_ids = spectral_df[id_col].sort_values().tolist()

        # Sidebar specific to Hyperspectral View
        st.sidebar.header("Plotting Controls")
        scenario = st.sidebar.radio(
            "Select Visualization Scenario:",
            ("Single Canopy", "Compare Multiple Canopies", "Average of Selection")
        )

        if scenario == "Single Canopy":
            st.subheader("Single Canopy Spectral Signature")
            selected_id = st.sidebar.selectbox("Select Canopy ID:", available_ids)
            
            canopy_data = spectral_df[spectral_df[id_col] == selected_id][band_cols].iloc[0].values
            
            fig = px.line(
                x=x_axis_labels, 
                y=canopy_data,
                labels={'x': 'Band Number', 'y': 'Mean Reflectance'},
                title=f"Spectral Signature for Canopy {selected_id}"
            )
            fig.update_traces(line_color='#00FFCC')
            st.plotly_chart(fig, use_container_width=True)

        elif scenario == "Compare Multiple Canopies":
            st.subheader("Comparative Spectral Signatures")
            selected_ids = st.sidebar.multiselect(
                "Select Canopy IDs to Compare:", 
                available_ids, 
                default=available_ids[:3] if len(available_ids) >= 3 else available_ids
            )
            
            if selected_ids:
                filtered_df = spectral_df[spectral_df[id_col].isin(selected_ids)]
                ...
                melted_df['Band_Number'] = melted_df['Band'].apply(lambda x: int(x.split('_')[1]))
                ...
                fig = px.line(
                    ...
                    y='Reflectance', 
                    color=id_col,
                    labels={'Band_Number': 'Band Number', 'Reflectance': 'Mean Reflectance', id_col: 'Canopy ID'},
                    title="Spectral Variance Across Selected Canopies"
                )
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.warning("Please select at least one canopy from the sidebar.")

        elif scenario == "Average of Selection":
            st.subheader("Mean Spectral Signature of Selected Group")
            selected_ids = st.sidebar.multiselect(
                "Select Canopy IDs for Group Average:", 
                available_ids, 
                default=available_ids[:10] if len(available_ids) >= 10 else available_ids
            )
            
            if selected_ids:
                filtered_df = spectral_df[spectral_df[id_col].isin(selected_ids)]
                mean_signature = filtered_df[band_cols].mean().values
                
                fig = px.line(
                    x=x_axis_labels, 
                    y=mean_signature,
                    labels={'x': 'Band Number', 'y': 'Average Reflectance'},
                    title=f"Mean Spectral Signature (n={len(selected_ids)} Canopies)"
                )
                fig.update_traces(line_color='#FF00FF', line_width=3)
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.warning("Please select canopies to calculate the average.")

        st.divider()
        st.subheader("Raw Reflectance Data Preview")
        st.dataframe(spectral_df.head(15))